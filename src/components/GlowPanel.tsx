...
export const GlowPanel: React.FC<GlowPanelProps> = ({
  ...
  // Adjust glow opacity and background for theme
  const effectiveGlowOpacity = isLight ? glowOpacity * 0.45 : glowOpacity;
  const effectiveBorderGradient = borderGradient || gradient;
  const effectiveHoverGlow = hoverGlowOpacity !== undefined
    ? (isLight ? hoverGlowOpacity * 0.5 : hoverGlowOpacity)
    : Math.min(effectiveGlowOpacity * 1.35, 0.65);
  const currentOpacity = hover && isHovered ? effectiveHoverGlow : effectiveGlowOpacity;

  const bgSolidColor = isLight ? '#FFFFFF' : '#111113';
    ...
    >
      ...
      <div
        className={`relative h-full w-full transition-all duration-200 ease-out ${
          isLight ? 'shadow-md shadow-slate-200/60' : ''
        }`}
        style={{
          borderRadius,
          background: `linear-gradient(${bgSolidColor}, ${bgSolidColor}) padding-box, ${effectiveBorderGradient} border-box`,
