...
export default function App() {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col font-sans selection:bg-cyan-500/30 selection:text-cyan-200 relative overflow-x-hidden transition-colors duration-200 ${
        isLight ? 'bg-slate-50 text-slate-900' : 'bg-[#0A0A0B] text-white'
      }`}
    >
      {/* BACKGROUND AMBIENT LIGHTING (Subtle fixed ambient gradients) */}
      ...
      {/* Footer */}
      <footer
        className="py-4 text-center text-xs font-mono relative z-10 transition-colors"
        style={{
          borderTop: isLight ? '1px solid rgba(226, 232, 240, 0.8)' : '1px solid rgba(255, 255, 255, 0.07)',
          background: isLight ? 'rgba(255, 255, 255, 0.85)' : 'rgba(10, 10, 11, 0.85)',
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className={isLight ? 'text-slate-700 font-medium' : 'text-slate-300 font-medium'}>
              Aritra's Vault Distributed Storage Prototype
            </span>
          </div>
